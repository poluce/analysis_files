...
AlgorithmCommand::AlgorithmCommand(IThermalAlgorithm* algorithm,
...
bool AlgorithmCommand::execute()
{
    if (!m_algorithm || !m_inputCurve || !m_curveManager) {
        qWarning() << "AlgorithmCommand::execute: 算法、曲线或CurveManager指针为空";
        return false;
    }
    ...
    try {
        ...
        if (outputData.isEmpty()) {
            qWarning() << "AlgorithmCommand::execute: 算法返回空数据";
            return false;
        }
        ...
        m_newCurveData.setSignalType(m_algorithm->getOutputSignalType(m_inputCurve->signalType()));
        ...
        m_executed = true;
        qDebug() << "AlgorithmCommand: 执行算法" << m_algorithmName
                 << "于曲线" << m_inputCurveName
                 << "，创建新曲线" << newName << "ID:" << m_newCurveId;

        return true;
    }
    catch (const std::exception& e) {
        qWarning() << "AlgorithmCommand::execute: 算法执行异常:" << e.what();
        return false;
    }
}
...
bool AlgorithmCommand::undo()
{
    ...
    if (m_curveManager->removeCurve(m_newCurveId)) {
        qDebug() << "AlgorithmCommand: 撤销算法" << m_algorithmName
                 << "，删除曲线" << m_newCurveData.name() << "ID:" << m_newCurveId;
        return true;
    } else {
...
bool AlgorithmCommand::redo()
{
    ...
    qDebug() << "AlgorithmCommand: 重做算法" << m_algorithmName
             << "，重新添加曲线" << m_newCurveData.name() << "ID:" << m_newCurveId;

    return true;
}

QString AlgorithmCommand::description() const
{
    return QString("对曲线'%1'执行%2").arg(m_inputCurveName, m_algorithmName);
}
