# 算法链式执行重构 - 任务执行清单

**基于文档**: `新设计文档/算法链式执行重构计划.md` (v1.2)
**执行日期**: 2025-11-19
**执行策略**: 0新类方案 - 不考虑向后兼容，直接全面切换

---

## 📋 任务总览

| 阶段 | 任务数 | 预计工时 | 状态 |
|------|--------|---------|------|
| 阶段1: 基础设施层 | 2 | 2.5h | ✅ 已完成 |
| 阶段2: 协调层 | 2 | 7h | ✅ 已完成 |
| 阶段3: 算法增强 | 2 | 2.5h | ⏸️ 待开始 |
| 阶段4: UI层调整 | 2 | 1.5-3h | ⏸️ 待开始 |
| 阶段5: 清理旧代码 | 1 | 1h | ⏸️ 待开始 |
| **总计** | **9** | **14.5-16h** | - |

---

## 🎯 阶段1: 基础设施层（无依赖）

### 任务1.1: OutputKeys 键生成工具增强 ✅ 已完成

**文件**: `Analysis/src/domain/algorithm/algorithm_result.h`

**改动清单**:
- [x] 删除 `LatestSuffix` 常量（第43-44行）
- [x] 删除 `ResultTypeSuffix` 常量（第46-47行）
- [x] 删除 `latestResult(algorithmName)` 函数（第66-68行）
- [x] 删除 `resultType(algorithmName)` 函数（第81-83行）
- [x] 新增 `ByTaskInfix` 常量
- [x] 新增 `ByCurveInfix` 常量
- [x] 新增 `LatestTaskIdSuffix` 常量
- [x] 新增 `HistoryTaskIdsSuffix` 常量
- [x] 新增 `byTask(algorithm, taskId)` 函数
- [x] 新增 `latestTaskId(algorithm, curveId)` 函数
- [x] 新增 `historyTaskIds(algorithm, curveId)` 函数

**验收标准**:
```cpp
// 测试键生成
QString key1 = OutputKeys::byTask("differentiation", "task-001");
QCOMPARE(key1, "result/differentiation/byTask/task-001");

QString key2 = OutputKeys::latestTaskId("differentiation", "curve-123");
QCOMPARE(key2, "result/differentiation/byCurve/curve-123/latestTaskId");

QString key3 = OutputKeys::historyTaskIds("differentiation", "curve-123");
QCOMPARE(key3, "result/differentiation/byCurve/curve-123/historyTaskIds");
```

---

### 任务1.2: AlgorithmContext 结果存取封装 ⏸️ 待开始

**文件**:
- `Analysis/src/application/algorithm/algorithm_context.h`
- `Analysis/src/application/algorithm/algorithm_context.cpp`

**改动清单**:
- [ ] 新增 `m_historyDepth` 成员变量（默认20）
- [ ] 新增 `saveResult(taskId, algorithm, parentCurveId, result)` 方法
- [ ] 新增 `latestResult(algorithm, curveId)` 方法
- [ ] 新增 `historyResults(algorithm, curveId, limit)` 方法
- [ ] 新增 `setHistoryDepth(depth)` / `historyDepth()` 方法

**实现要点**:

**`saveResult()` 实现逻辑**:
```cpp
void AlgorithmContext::saveResult(
    const QString& taskId,
    const QString& algorithm,
    const QString& parentCurveId,
    const AlgorithmResult& result)
{
    // 1. 主存储（byTask）- 不可覆盖
    QString taskKey = OutputKeys::byTask(algorithm, taskId);
    setValue(taskKey, QVariant::fromValue(result));

    // 2. 更新最新任务ID指针
    QString latestKey = OutputKeys::latestTaskId(algorithm, parentCurveId);
    setValue(latestKey, taskId);

    // 3. 更新历史任务ID队列（LRU）
    QString historyKey = OutputKeys::historyTaskIds(algorithm, parentCurveId);
    QStringList history = get<QStringList>(historyKey).value_or(QStringList());

    // 追加到队列头部
    history.prepend(taskId);

    // LRU 裁剪：超出深度时删除旧任务
    if (history.size() > m_historyDepth) {
        QStringList toRemove = history.mid(m_historyDepth);
        history = history.mid(0, m_historyDepth);

        // 可选：删除超出深度的主存储
        for (const QString& oldTaskId : toRemove) {
            QString oldKey = OutputKeys::byTask(algorithm, oldTaskId);
            remove(oldKey);
        }
    }

    setValue(historyKey, history);

    qInfo() << "[AlgorithmContext] 保存结果:"
            << "taskId=" << taskId
            << "algorithm=" << algorithm
            << "curveId=" << parentCurveId
            << "历史深度=" << history.size();
}
```

**`latestResult()` 实现逻辑**:
```cpp
std::optional<AlgorithmResult> AlgorithmContext::latestResult(
    const QString& algorithm,
    const QString& curveId) const
{
    // 1. 读取最新任务ID指针
    QString latestKey = OutputKeys::latestTaskId(algorithm, curveId);
    auto taskIdOpt = get<QString>(latestKey);

    if (!taskIdOpt.has_value() || taskIdOpt.value().isEmpty()) {
        return std::nullopt;
    }

    // 2. 展开 taskId → 读取主存储
    QString taskId = taskIdOpt.value();
    QString taskKey = OutputKeys::byTask(algorithm, taskId);

    return get<AlgorithmResult>(taskKey);
}
```

**`historyResults()` 实现逻辑**:
```cpp
QVector<AlgorithmResult> AlgorithmContext::historyResults(
    const QString& algorithm,
    const QString& curveId,
    int limit) const
{
    // 1. 读取历史任务ID队列
    QString historyKey = OutputKeys::historyTaskIds(algorithm, curveId);
    auto historyOpt = get<QStringList>(historyKey);

    if (!historyOpt.has_value()) {
        return QVector<AlgorithmResult>();
    }

    QStringList taskIds = historyOpt.value();

    // 2. 限制返回数量
    if (limit > 0 && taskIds.size() > limit) {
        taskIds = taskIds.mid(0, limit);
    }

    // 3. 展开 taskId 列表 → 读取主存储
    QVector<AlgorithmResult> results;
    for (const QString& taskId : taskIds) {
        QString taskKey = OutputKeys::byTask(algorithm, taskId);
        auto resultOpt = get<AlgorithmResult>(taskKey);

        if (resultOpt.has_value()) {
            results.append(resultOpt.value());
        }
    }

    return results;
}
```

**验收标准**:
```cpp
// 测试并发保存
context->saveResult("task-001", "differentiation", "curve-123", result1);
context->saveResult("task-002", "differentiation", "curve-123", result2);
context->saveResult("task-003", "differentiation", "curve-123", result3);

// 验证最新结果
auto latest = context->latestResult("differentiation", "curve-123");
QVERIFY(latest.has_value());
QCOMPARE(latest->algorithmKey(), "differentiation");

// 验证历史结果
auto history = context->historyResults("differentiation", "curve-123");
QCOMPARE(history.size(), 3);

// 验证按时间倒序：[result3, result2, result1]
```

**LRU 裁剪测试**:
```cpp
context->setHistoryDepth(3);

// 执行 5 次
for (int i = 0; i < 5; ++i) {
    QString taskId = QString("task-%1").arg(i);
    context->saveResult(taskId, "differentiation", "curve-123", result);
}

auto history = context->historyResults("differentiation", "curve-123");
QCOMPARE(history.size(), 3);  // 只保留最近 3 次
```

---

## 🎯 阶段2: 协调层

### 任务2.1: AlgorithmCoordinator 信号机制重构 ⏸️ 待开始

**文件**:
- `Analysis/src/application/algorithm/algorithm_coordinator.h`
- `Analysis/src/application/algorithm/algorithm_coordinator.cpp`

**改动清单**:

**删除**:
- [ ] 删除 `saveResultToContext()` 方法（头文件 ~254行，cpp ~95-108行）
- [ ] 删除 `onAlgorithmResultReady()` 槽函数（cpp ~110-115行）
- [ ] 删除 `algorithmSucceeded(QString)` 信号（头文件 ~122行）

**新增**:
- [ ] 新增 `algorithmCompleted(taskId, algorithmName, parentCurveId, result)` 信号
- [ ] 新增 `WorkflowStatus` 枚举（Idle, Running, Completed, Failed, Cancelled）
- [ ] 新增 `PendingWorkflow` 结构体（类内私有）
- [ ] 新增 `runWorkflow(steps, curveIds)` 方法
- [ ] 新增 `cancelWorkflow(workflowId)` 方法
- [ ] 新增 `advanceWorkflow(taskId, algorithmName, result)` 私有方法
- [ ] 新增 `workflowCompleted(workflowId, outputCurveIds)` 信号
- [ ] 新增 `workflowFailed(workflowId, errorMessage)` 信号

**修改**:
- [ ] 重命名 `onAlgorithmResultReady()` → `onSyncAlgorithmResultReady()`
- [ ] 修改 `onSyncAlgorithmResultReady()` 实现：
  - 生成 taskId = `{algorithmName}-{timestamp}-{uuid}`
  - 调用 `context->saveResult(taskId, algorithmName, parentCurveId, result)`
  - 发射 `algorithmCompleted(taskId, algorithmName, parentCurveId, result)`
  - 调用 `advanceWorkflow(taskId, algorithmName, result)`
- [ ] 修改 `onAsyncAlgorithmFinished()` 实现（头文件 ~155行，cpp ~149-167行）：
  - 调用 `context->saveResult(taskId, algorithmName, parentCurveId, result)`
  - 删除 `emit algorithmSucceeded(algorithmName)`
  - 发射 `algorithmCompleted(taskId, algorithmName, parentCurveId, result)`
  - 调用 `advanceWorkflow(taskId, algorithmName, result)`

**PendingWorkflow 结构定义**:
```cpp
struct PendingWorkflow {
    QString id;                         // 工作流实例ID
    QStringList steps;                  // 算法步骤列表
    int currentStepIndex = 0;           // 当前执行到第几步
    QStringList inputCurveIds;          // 初始输入曲线
    QHash<int, QStringList> stepOutputs; // 每步的输出曲线ID
    WorkflowStatus status = WorkflowStatus::Idle;
    QString errorMessage;
};
```

**验收标准**:
```cpp
// 测试单次执行信号
QSignalSpy spy(coordinator, &AlgorithmCoordinator::algorithmCompleted);
coordinator->executeAlgorithm("differentiation", "curve-001");
QCOMPARE(spy.count(), 1);

QList<QVariant> args = spy.takeFirst();
QCOMPARE(args.at(1).toString(), "differentiation");  // algorithmName
QCOMPARE(args.at(2).toString(), "curve-001");        // parentCurveId
```

---

### 任务2.2: 工作流实现 ⏸️ 待开始

**改动清单**:
- [ ] 实现 `runWorkflow(steps, curveIds)` 方法
  - 生成工作流 ID: `workflow-{timestamp}-{uuid}`
  - 初始化 `PendingWorkflow` 状态
  - 执行第一步算法
- [ ] 实现 `advanceWorkflow(taskId, algorithmName, result)` 方法
  - 检查是否有活动工作流
  - 验证算法名匹配当前步骤
  - 记录当前步骤输出
  - 推进到下一步或完成工作流
- [ ] 实现 `cancelWorkflow(workflowId)` 方法
  - 标记状态为 Cancelled
  - 清理工作流状态
  - 发射 `workflowFailed` 信号

**`advanceWorkflow()` 实现逻辑**:
```cpp
void AlgorithmCoordinator::advanceWorkflow(
    const QString& taskId,
    const QString& algorithmName,
    const AlgorithmResult& result)
{
    if (!m_currentWorkflow.has_value()) {
        return;  // 无活动工作流
    }

    auto& wf = m_currentWorkflow.value();

    // 验证算法名匹配当前步骤
    if (wf.currentStepIndex >= wf.steps.size()) {
        return;  // 已超出步骤范围
    }

    QString expectedAlgorithm = wf.steps[wf.currentStepIndex];
    if (algorithmName != expectedAlgorithm) {
        return;  // 算法名不匹配
    }

    // 记录当前步骤的输出曲线ID
    QStringList outputCurveIds;
    if (result.hasCurves()) {
        for (const ThermalCurve& curve : result.curves()) {
            outputCurveIds.append(curve.id());
        }
    }
    wf.stepOutputs[wf.currentStepIndex] = outputCurveIds;

    // 推进到下一步
    wf.currentStepIndex++;

    // 检查是否完成
    if (wf.currentStepIndex >= wf.steps.size()) {
        // 工作流完成
        wf.status = WorkflowStatus::Completed;
        emit workflowCompleted(wf.id, wf.stepOutputs[wf.steps.size() - 1]);
        m_currentWorkflow.reset();
        qInfo() << "[AlgorithmCoordinator] 工作流完成:" << wf.id;
        return;
    }

    // 执行下一步
    QString nextAlgorithm = wf.steps[wf.currentStepIndex];
    QString nextInputCurve = outputCurveIds.isEmpty() ? wf.inputCurveIds.first() : outputCurveIds.first();

    qInfo() << "[AlgorithmCoordinator] 工作流推进:"
            << "步骤" << (wf.currentStepIndex + 1) << "/" << wf.steps.size()
            << "算法=" << nextAlgorithm;

    executeAlgorithm(nextAlgorithm, nextInputCurve);
}
```

**验收标准**:
```cpp
// 测试线性工作流
QSignalSpy completedSpy(coordinator, &AlgorithmCoordinator::workflowCompleted);

QString wfId = coordinator->runWorkflow(
    {"movingAverage", "differentiation"},
    {"curve-001"}
);

// 等待工作流完成
QVERIFY(completedSpy.wait(5000));
QCOMPARE(completedSpy.count(), 1);

QList<QVariant> args = completedSpy.takeFirst();
QCOMPARE(args.at(0).toString(), wfId);

// 验证生成了两条曲线（平滑 + 微分）
QStringList outputs = args.at(1).toStringList();
QCOMPARE(outputs.size(), 1);  // 最终输出是微分曲线
```

---

## 🎯 阶段3: 算法增强

### 任务3.1: AlgorithmDescriptor 依赖声明 ⏸️ 待开始

**文件**: 所有算法实现（5个）

**改动清单**:
- [ ] **DifferentiationAlgorithm**:
  - `prerequisites = {ContextKeys::ActiveCurve}`
  - `produces = {"curve"}`
- [ ] **IntegrationAlgorithm**:
  - `prerequisites = {ContextKeys::ActiveCurve}`
  - `produces = {"curve"}`
- [ ] **MovingAverageFilterAlgorithm**:
  - `prerequisites = {ContextKeys::ActiveCurve}`
  - `produces = {"curve"}`
- [ ] **BaselineCorrectionAlgorithm**:
  - `prerequisites = {ContextKeys::ActiveCurve, ContextKeys::SelectedPoints}`
  - `produces = {"curve"}`
- [ ] **PeakAreaAlgorithm**:
  - `prerequisites = {ContextKeys::ActiveCurve, ContextKeys::SelectedPoints}`
  - `produces = {"markers", "scalar", "region"}`

**说明**:
- `prerequisites`: 使用 `ContextKeys` 常量，表示算法执行前必须存在的上下文数据
- `produces`: 结果类型契约（如 `"curve"`, `"markers"`, `"scalar"`）

**验收标准**:
```cpp
auto desc = algorithm->descriptor();
QVERIFY(!desc.prerequisites.isEmpty());
QVERIFY(!desc.produces.isEmpty());

// 验证 prerequisites 使用 ContextKeys
QCOMPARE(desc.prerequisites.first(), ContextKeys::ActiveCurve);
```

---

### 任务3.2: 依赖校验实现 ⏸️ 待开始

**文件**: `algorithm_coordinator.cpp`

**改动清单**:
- [ ] 新增 `checkPrerequisites(algorithmName)` 私有方法
- [ ] 在 `executeAlgorithm()` 前调用校验
- [ ] 缺失依赖时发出 `showMessage` 信号

**`checkPrerequisites()` 实现逻辑**:
```cpp
bool AlgorithmCoordinator::checkPrerequisites(const QString& algorithmName)
{
    auto algorithm = m_algorithmManager->getAlgorithm(algorithmName);
    if (!algorithm) {
        qWarning() << "[AlgorithmCoordinator] 算法不存在:" << algorithmName;
        return false;
    }

    AlgorithmDescriptor desc = algorithm->descriptor();

    for (const QString& prereq : desc.prerequisites) {
        if (!m_context->contains(prereq)) {
            QString msg = QString("算法 '%1' 缺少前置依赖: %2")
                         .arg(desc.displayName)
                         .arg(prereq);
            qWarning() << "[AlgorithmCoordinator]" << msg;
            emit showMessage(msg);
            return false;
        }
    }

    return true;
}
```

**验收标准**:
```cpp
// 测试依赖校验
curveManager->clearAll();  // 清空曲线

bool canRun = coordinator->checkPrerequisites("differentiation");
QVERIFY(!canRun);  // 缺少 activeCurve

curveManager->addCurve(curve);
curveManager->setActiveCurve("curve-001");

canRun = coordinator->checkPrerequisites("differentiation");
QVERIFY(canRun);  // 依赖满足
```

---

## 🎯 阶段4: UI层调整

### 任务4.1: MainController 信号连接 ⏸️ 待开始

**文件**:
- `Analysis/src/ui/controller/main_controller.h`
- `Analysis/src/ui/controller/main_controller.cpp`

**改动清单**:

**删除**:
- [ ] 删除 `onCoordinatorAlgorithmSucceeded()` 槽函数（cpp ~476-485行）
- [ ] 删除 `algorithmSucceeded` 信号的 connect 语句（cpp ~165-198行）

**新增**:
- [ ] 新增 `onAlgorithmCompleted(taskId, algorithmName, parentCurveId, result)` 槽函数
- [ ] 新增 `onWorkflowCompleted(workflowId, outputCurveIds)` 槽函数
- [ ] 新增 `onWorkflowFailed(workflowId, errorMessage)` 槽函数
- [ ] 连接 `algorithmCompleted` 信号
- [ ] 连接 `workflowCompleted` 信号
- [ ] 连接 `workflowFailed` 信号

**`onAlgorithmCompleted()` 实现**:
```cpp
void MainController::onAlgorithmCompleted(
    const QString& taskId,
    const QString& algorithmName,
    const QString& parentCurveId,
    const AlgorithmResult& result)
{
    Q_ASSERT(m_initialized);

    qInfo() << "算法执行完成:" << algorithmName
            << "taskId:" << taskId
            << "源曲线:" << parentCurveId
            << "结果类型:" << static_cast<int>(result.type());

    cleanupProgressDialog();
    m_currentTaskId.clear();
    m_currentAlgorithmName.clear();

    // 可选：自动化工作流决策
    // if (algorithmName == "differentiation") {
    //     QString dtgCurveId = result.outputCurveIds().first();
    //     m_algorithmCoordinator->executeAlgorithm("peakDetection", dtgCurveId);
    // }
}
```

**验收标准**:
```cpp
// 测试信号连接
QSignalSpy spy(controller, SIGNAL(/* 内部信号 */));
coordinator->executeAlgorithm("differentiation", "curve-001");
// 验证 controller 的槽函数被调用
```

---

### 任务4.2: ChartView 区域叠加能力确认 ⏸️ 待开始

**改动清单**:
- [ ] 检查 `ChartView` 是否已有 `onRegionsGenerated()` 槽函数
- [ ] 检查 `ThermalChart` 是否已有区域叠加方法（如 `addCurveRegions()`）
- [ ] 若未实现，则需补充最小可用 API

**如果需要实现**:

**ChartView 新增**:
```cpp
class ChartView : public QWidget {
public slots:
    void onRegionsGenerated(const QString& curveId,
                           const QVector<RegionData>& regions);
};
```

**ThermalChart 新增**:
```cpp
class ThermalChart : public QChart {
public:
    void addCurveRegions(const QString& curveId,
                        const QVector<RegionData>& regions);
    void clearCurveRegions(const QString& curveId);
    void clearAllRegions();

private:
    QHash<QString, QVector<QGraphicsPolygonItem*>> m_curveRegions;
};
```

**验收标准**:
```cpp
// 测试区域添加
QVector<RegionData> regions;
RegionData region;
region.polygon = QPolygonF({QPointF(100, 0), QPointF(200, 0),
                            QPointF(200, 10), QPointF(100, 5)});
region.fillColor = QColor(255, 0, 0, 80);
region.label = "峰面积: 123.45 J/g";
regions.append(region);

chartView->onRegionsGenerated("curve-001", regions);

// 验证区域已添加到图表
QVERIFY(chart->curveRegions("curve-001").size() == 1);

// 验证区域随曲线删除
curveManager->removeCurve("curve-001");
QVERIFY(chart->curveRegions("curve-001").isEmpty());
```

---

## 🎯 阶段5: 清理旧代码

### 任务5.1: 清理 Context 中的旧数据 ⏸️ 待开始

**改动清单**:
- [ ] 搜索并删除所有使用 `OutputKeys::latestResult(algorithmName)` 的代码
- [ ] 搜索并删除所有使用 `OutputKeys::resultType(algorithmName)` 的代码
- [ ] 搜索 `get.*latestResult` 确认无遗留调用

**验收命令**:
```bash
# 搜索确认
grep -r "latestResult" --include="*.cpp" --include="*.h" Analysis/src/
# 应该只在 algorithm_context.cpp 的新实现中出现

grep -r "resultType" --include="*.cpp" --include="*.h" Analysis/src/
# 应该只在 MetaKeys 命名空间中出现
```

---

## 🧪 测试验证清单

### 单元测试

- [ ] **测试1**: 并发执行（无覆盖）
- [ ] **测试2**: LRU 裁剪
- [ ] **测试3**: 工作流执行
- [ ] **测试4**: 依赖校验

### 集成测试

- [ ] **测试5**: 端到端工作流

---

## 📝 验收标准

### 功能验收

- [ ] **并发执行**: 同时执行 3 次微分，结果互不覆盖
- [ ] **历史查询**: 可以查询某条曲线某算法的最近 20 次执行
- [ ] **LRU 裁剪**: 执行 25 次后只保留最近 20 次
- [ ] **工作流执行**: 成功执行 2-3 步线性工作流
- [ ] **工作流取消**: 可以在执行过程中取消
- [ ] **依赖校验**: 缺少前置依赖时提示错误
- [ ] **信号机制**: 所有算法完成时发射 `algorithmCompleted` 信号

### 代码质量验收

- [ ] **无编译警告**: 编译时无 warning
- [ ] **无旧代码残留**: 搜索确认无 `algorithmSucceeded`/旧 `latestResult`
- [ ] **无硬编码键名**: 所有 Context 访问使用 OutputKeys
- [ ] **单元测试覆盖**: 核心方法测试覆盖率 > 80%
- [ ] **集成测试通过**: 端到端工作流测试通过

### 性能验收

- [ ] **存储性能**: 1000 次执行的保存时间 < 100ms
- [ ] **查询性能**: 历史查询（10条）时间 < 10ms
- [ ] **工作流延迟**: 3步工作流的总开销 < 50ms（不含算法执行）

---

## 📊 进度追踪

| 日期 | 完成任务 | 备注 |
|------|---------|------|
| 2025-11-19 | 任务1.1: OutputKeys 增强 | ✅ 已完成 |
| 2025-11-19 | 任务1.2: AlgorithmContext 结果存取封装 | ✅ 已完成 |
| 2025-11-19 | 阶段1: 基础设施层 | ✅ 全部完成 |
| 2025-11-19 | 任务2.1: AlgorithmCoordinator 信号机制重构 | ✅ 已完成 |
| 2025-11-19 | 任务2.2: 工作流实现 | ✅ 已完成 |
| 2025-11-19 | 阶段2: 协调层 | ✅ 全部完成 |

---

## 🚨 风险清单

| 风险 | 概率 | 影响 | 缓解措施 |
|------|------|------|---------|
| 信号连接遗漏 | 中 | 高 | 编译时搜索所有 `algorithmSucceeded` 确认删除干净 |
| Context 键名冲突 | 低 | 中 | 使用 OutputKeys 统一生成，避免硬编码字符串 |
| 工作流状态混乱 | 中 | 中 | 单实例限制，添加状态日志，单元测试覆盖 |
| LRU 裁剪丢失重要数据 | 低 | 低 | 配置可调（默认20），关键任务不裁剪 |

---

**最后更新**: 2025-11-19
